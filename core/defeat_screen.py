...
class DefeatScreen(QWidget):
    def __init__(self, wanted_word):
        ...
        # [Title]
        self.title = QLabel(f"{self.language['defeat']} {wanted_word}")
        self.title.setObjectName("defeatTitle")
        self.title.setWordWrap(False)

        # [Images]
        self.image = QLabel()
        self.pixmap = QPixmap(str(get_assets_route("assets/lost_hangman.png"))).scaled(350, 350, Qt.KeepAspectRatio, Qt.SmoothTransformation)
...
if __name__ == "__main__":
    ...
    window = DefeatScreen("hesitation")
    window.show()
    sys.exit(app.exec_())